...
it is), §15 (the envelope as it now stands). Then `LESSONS_LEARNED.md` §8, §7 and
§10, which are mostly about the difference between a check passing and the thing
...
doors that swing open on a wax seal, with the paper and the wax generated by SVG filters
rather than drawn as gradients. Plan §15 and LESSONS_LEARNED §10. If you are touching
that page, read §10 first: four of its five bugs were invisible to every assertion and
...
4. **TAP-7762 photography.** Needs somebody with a camera, not a keyboard. The
   invitation hero is still a stock photograph of another couple; the save-the-date card
   now carries `beach-fire.jpg` full-bleed behind its type, which is a driftwood fire and
   not Port Aransas. `photo_credit_for()` builds each page's credit from the filenames
   the template names, so swapping a CC BY image in makes a credit line appear by itself
   — and that has now happened: the save-the-date renders one where it used to render
   none. Any replacement has to clear 4.5:1 against the **darkest** part of the picture,
   not its average.
...
6. **TAP-7809 filter cost.** The save-the-date's paper and wax are generated by SVG
   filters rather than shipped as images. They are all static, so each renders once and
   the reveal only composites transforms — but that is reasoning, not a measurement, and
   the guest list skews old, which correlates with older phones. Measure first paint on
   real low-end hardware before deciding anything.
7. **A dependency lockfile.** There is none, so a production image rebuilt in 2028 may
...
- **`z-index` is only meaningful inside a stacking context, and several innocuous
  properties create one** — `transform`, `opacity`, `filter`, `transform-style:
  preserve-3d`, `isolation`. `preserve-3d` is worse than the rest: it also sorts its
  children by position in space rather than by `z-index`, so a child's `z-index: 2` beat
  an entire 3D subtree and the envelope's doors were simply not drawn. When paint order
  is wrong, find the nearest ancestor that creates a context before adjusting numbers.
- **A viewport-dependent defect needs a viewport-parameterised test.** The doors of the
  envelope swung off the side of a phone and were clipped away, so the liner — the whole
  reason for the rebuild — was invisible at 390px. The test ran at 1440px only and
  passed. Nearly everything in `test_visual.py` is already parameterised over both
  widths; a new test that is not should be a deliberate choice.
- **Hit-testing skips `pointer-events: none`, including `elementsFromPoint`.** Two new
  tests confidently reported a bare card on a sealed, perfectly opaque envelope, because
  the envelope is correctly `pointer-events: none` and the probe could not see it. If a
  test contradicts what the screenshot plainly shows, suspect the instrument.
